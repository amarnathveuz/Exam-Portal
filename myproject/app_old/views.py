    ...
    if request.method == "POST":
        ...
        for section_title,section_count in main_zip:
            ...
            for  question,model_count in question_zip:
                ...
                question_add_line_zip = zip(choice_data,question_line_count)
                for  choice_data,choice_count in question_add_line_zip:

                    addline_check = request.POST.get("addline_check"+str(choice_count),False)
                    try:
                        file_data = request.FILES['question_image_choice'+str(choice_count)]
                    except:
                        print("nulllllllllll=------------------")
                        pass
...
def open_section_based_question(request):
    ...
    return render(request,'open_section_based_question.html',{'value1':value1,'data_id':data_id,'status':status,'modal_id':modal_id})
